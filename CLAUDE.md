...
- `aiAgent.js` — **סוכן עם כלים** (tool-use של Anthropic): חיפוש אירועים, פרטי אירוע, מסמכים פתוחים, חיפוש לקוח, תצוגה מקדימה להצעה, הפקת הצעת מחיר, שליחת מסמך, ו-`remember`/`forget` לזיכרון. `POST /api/agent/chat`, כפתור 🤖 צף. שלוש גדרות: כלים כותבים רק ל-`role==='admin'` (`WRITE_TOOLS` מסוננים גם מרשימת הכלים וגם בהרצה), כל כלי עובר `companyEvents(db, companyId)` ורץ בתוך `withCompany`, ו-`MAX_ROUNDS` חוסם לולאת כלים. **אין כלי שמפיק מסמך מס** — 10 בלבד. `buildQuote` משותף לתצוגה המקדימה ולהפקה, כדי שמה שנראה יהיה מה שיופק. פונקציות של server.js מוזרקות דרך `registerAgentHost` (מניעת import מעגלי).
  - **זיכרון:** `db.agentMemory[companyId]` (ו-`__all` לכלל העסקים), מוזרק להנחיות בכל שיחה. נכנס משתי דרכים — הכלי `remember`, ו-`learnFromTurn()` שרץ **אחרי** שהתשובה נשלחה (Haiku, לא חוסם, מנהל בלבד) ומחלץ לקחים גם כשלא נאמר "תזכור". דדופ לפי חפיפת מילים 75% — פריט באותו נושא **מחליף** ולא מצטבר, אחרת נוצרים זיכרונות סותרים. תקרה `MEM_MAX=80`. הכל גלוי ב-🧠 בחלונית ו-`/api/agent/memory`.
...
- **סגירת מסמך מקור:** `linkedDocumentIds` נקבע **ביצירה בלבד** — אין קישור בדיעבד. מסמך המשך שהופק ישירות בחשבונית ירוקה משאיר את הצעת המחיר/חשבון העסקה פתוחים שם לנצח, גם כשאצלנו האירוע מחויב ושולם. שני דוחות בפרטי העסק סוגרים את הפער: `/api/stale-quotes` (10) ו-`/api/stale-proformas` (300), שניהם עם `/close` שדורש `confirm`. **חשוב:** `staleProformas` נקראת מ-`openDocuments()` של חשבונית ירוקה ולא מהדגל `converted` שלנו — הדגל נקבע גם מזיהוי לתצוגה, וסגירה על סמכו תסגור מסמכים חיים.
...
## הצעות התאמה בבנק — תצלום קפוא (21.08.26)
`suggestions[]` נשמר על שורת הבנק בזמן הייבוא או "↻ רענן הצעות", ואינו מתעדכן מעצמו.
לכן מסמך שנמחק המשיך להופיע ככפתור הצעה. `dropDocFromBank(db, docId, companyId)`
בשרת מנתק מסמך משני המקומות — `matchedInvoices` **וגם** `suggestions` — ומחזיר
ל-`unmatched` שורה שנשארה בלי מסמך (שורה `ignored` נשארת מוסתרת). נקרא משני מסלולי
המחיקה. בנוסף, לפני שיוך מאמתים ב-GI שההוצאה עדיין קיימת — 404 בלבד חוסם (409 + ניקוי),
כדי שתקלת רשת לא תמנע שיוך תקין.
> **כלל:** ראוט חדש שמוחק מסמך חייב לקרוא ל-`dropDocFromBank`. ניקוי `matchedInvoices` בלבד משאיר הצעה יתומה.

## קודי שגיאה של חשבונית ירוקה
- **`errorCode 1010` = כפילות** — כבר קיימת הוצאה של אותו ספק עם אותו מספר מסמך.
  ה-`errorMessage` הוא **המזהה של ההוצאה הקיימת**, לא טקסט שגיאה. ביצירה זה מטופל
  כ"כבר נקלטה"; בעדכון (`PUT /api/expenses/:id`) מתורגם להודעה בעברית עם ספק/תאריך/סכום
  וכפתור לפתיחת ההוצאה הקיימת. אין להציג פלט API גולמי למשתמש.
