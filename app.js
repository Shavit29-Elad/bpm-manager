...
// אחרי כל פעולה (כתיבה) — חלון שבו כל הקריאות נשלפות טרי מהשרת ולא ממטמון. מטרתו לתפוס עדכוני חשבונית ירוקה מיד,
// גם כשה-API של חשבונית ירוקה מאנדקס מסמך חדש בהשהיה של כמה שניות (אחרת נתפסת תשובה ישנה ומוקפאת ל-60 שניות).
...
(() => {
  ...
  window.fetch = (input, init) => {
    ...
    if (method !== 'GET' && url.indexOf('/api/') !== -1) pr.then(() => { _lastWriteAt = Date.now(); clearApiCache(); }).catch(() => {});
    return pr;
  };
})();
...
const TAB_RENDERERS = { home: renderHome, summary: renderBusinessSummary, events: renderCombined, clients: renderClients,
  ...
  payroll: renderPayroll, vehicles: renderVehicles, eventsboard: renderEventsBoard, business: renderBusiness };
let _renderGen = 0;
...
window.openIncomeBreakdown = () => {
  ...
  _figModal('incBreakModal', `<div class="modal-card" style="width:min(640px,95vw)">
    ...
    <div style="margin-top:12px;border:1px solid var(--line);border-radius:8px;padding:10px">
      <div style="font-weight:700;font-size:13px;margin-bottom:6px">מסמכים בתאריך עתידי: ${f.futureCount ? `${f.futureCount} · סה"כ ${money(f.futureExVat)} לפני מע"מ` : 'אין'}</div>
      ${!f.futureCount ? `<div class="muted" style="font-size:12.5px;line-height:1.6">כל המסמכים השנה מתוארכים עד היום.
        כלומר אם עדיין יש הפרש מול חשבונית ירוקה, הוא <b>לא</b> נובע מטווח התאריכים — ואפשר לפסול את ההסבר הזה.</div>` : `
...
async function renderContractors(c) {
  ...
  if (inp) {
    ...
    // הערה: לא ממקדים אוטומטית את שדה החיפוש — פוקוס בעת רינדור-מחדש (למשל אחרי קליטת חשבונית) גרם לגלילה למטה לרשימת הספקים.
  }
...
function draftsSection() {
  ...
  return `<div class="row-between"><div><h2>🧾 טיוטות הוצאה לאישור</h2>
    ...
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:10px;padding:9px 11px;background:var(--panel2);border:1px solid var(--line);border-radius:10px">
...
async function renderLinkedNow() {
  ...
  box.innerHTML = `<div style="border:1px solid var(--accent2);border-radius:10px;overflow:hidden;margin-bottom:10px;background:#f0fdf4">
    ...
    ${evs.map(ev => `<div style="display:flex;gap:7px;align-items:center;flex-wrap:wrap;font-size:12.5px;padding:6px 10px;border-top:1px solid var(--line)">
      ...
      ${ev.supplierPaid ? '' : `<button class="btn ghost" style="padding:2px 8px;font-size:11.5px;color:var(--accent2);white-space:nowrap" onclick="markSupplierPaidEvent('${ev.eventId}',${ev.index},this)" title="סימון ידני: שילמת לספק בפועל (מחוץ למעקב הבנק)">✓ סמן ששולם ידנית</button>`}
...
window.filterApSup = (q) => {
  const sel = document.getElementById('apSup'); if (!sel) return;
  const t = String(q || '').trim().toLowerCase();
  [...sel.options].forEach(o => { if (!o.value) return; o.style.display = (!t || o.textContent.toLowerCase().includes(t)) ? '' : 'none'; });
};
...
window.exeRenderEvents = () => {
  ...
  if (!ctx.addFilter) { const now = new Date(); ctx.addFilter = { month: String(now.getMonth() + 1).padStart(2, '0'), year: String(now.getFullYear()) }; }
  const rowHtml = (ev, inputId, amt, isNew) => `<div style="display:flex;gap:8px;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px solid var(--line)">
      <span style="font-size:13px;flex:1">${isNew ? '🆕 ' : ''}${escapeHtml(`${ev.artist || ev.clientName || ev.client || 'אירוע'} · ${ddmy(ev.date)}${ev.location ? ' · ' + ev.location : ''}`)}</span>
  ...
  const years = [...new Set(ctx.allEvents.map(e => String(e.date || '').slice(0, 4)).filter(Boolean))].sort().reverse();
  const monthOpts = `<option value="all">כל החודשים</option>` + Array.from({ length: 12 }, (_, i) => { const mm = String(i + 1).padStart(2, '0'); return `<option value="${mm}" ${ctx.addFilter.month === mm ? 'selected' : ''}>${MONTHS_HE[i]}</option>`; }).join('');
  const yearOpts = `<option value="all">כל השנים</option>` + years.map(y => `<option value="${y}" ${ctx.addFilter.year === y ? 'selected' : ''}>${y}</option>`).join('');
  box.innerHTML = `<div style="font-size:13px;font-weight:700;margin-bottom:4px">💰 סכום לספק לכל אירוע</div>
    ...
    <div style="display:flex;gap:6px;margin-top:8px">
      <select id="exeAddMonth" onchange="exeSetAddFilter()" style="flex:1;font-size:12px" title="חודש">${monthOpts}</select>
      <select id="exeAddYear" onchange="exeSetAddFilter()" style="flex:1;font-size:12px" title="שנה">${yearOpts}</select>
    </div>
    <div style="margin-top:6px"><select id="exeAddSelect" style="width:100%;font-size:12px"></select></div>
...
// בונה מחדש רק את אפשרויות בורר "הוסף אירוע" לפי סינון החודש/שנה הנבחר (לא נוגע בשורות שכבר משויכות)
window.exeRenderAddOptions = () => {
  const ctx = window._exeCtx; if (!ctx) return;
  ...
  sel.innerHTML = `<option value="">➕ הוסף אירוע לשיוך לספק${list.length ? '' : ' (אין אירועים בחודש זה)'}…</option>${opts}`;
};
window.exeSetAddFilter = () => {
  const ctx = window._exeCtx; if (!ctx) return;
  ctx.addFilter = { month: document.getElementById('exeAddMonth')?.value || 'all', year: document.getElementById('exeAddYear')?.value || 'all' };
  exeRenderAddOptions();
};
...
// ---- לוח אירועים (משה כורסיה) ----
// 14 תפקידים קבועים לכל אירוע, ספק ומחיר לכל אחד, וסיכום הכנסות מול הוצאות
// לפי חודש ולפי אירוע. השורות נשמרות באותו מבנה של "ספקים לתשלום", ולכן ספק
// שנוסף כאן נכנס אוטומטית למעקב התשלומים ולהתאמות הבנק.
const BOARD_ROLES = ['מנהלת הצגה', 'קלידן', 'מתופף', 'גיטריסט', 'בסיסט', 'סאונדמן', 'בקליינר',
...
async function renderEventsBoard(c) {
  const _g = rgen(c);
  const year = _boardYear || new Date().getFullYear();
  const r = await api(`/api/event-board?year=${year}`).catch(() => ({ error: 'שגיאת רשת' }));
  if (rstale(c, _g)) return; // הכרטיסייה הוחלפה בזמן הטעינה — לא כותבים על החדשה
  if (!r || r.error) {
  ...
  c.innerHTML = `
    <div class="panel">
      <div class="row-between">
        <div><h2>🎭 לוח אירועים — ${year}</h2><span class="muted">הכנסות והוצאות לכל אירוע, מקובץ לפי חודשים</span></div>
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <select onchange="boardSetYear(this.value)" style="padding:6px 10px">${yrs.map(y => `<option value="${y}" ${String(y) === String(year) ? 'selected' : ''}>${y}</option>`).join('')}</select>
          <button class="btn primary" onclick="openBoardEdit()">➕ הוספת אירוע</button>
        </div>
      </div>
      <div class="cards" style="margin-top:14px">
...
function boardMonthPanel(m) {
  const rows = m.events.map(ev => {
    ...
    return `<tr style="cursor:pointer${selOn ? ';background:#f0f7ff' : ''}" onclick="openBoardView('${ev.id}')">
      ...
      <td data-label="מחיר ללקוח" style="text-align:left;white-space:nowrap">${money(t.clientPriceEx || 0)}${t.commissionEx ? `<div class="muted" style="font-size:10.5px;color:var(--warn)">${(t.commissions || []).length > 1 ? `${(t.commissions || []).length} עמלות` : `עמלה ${t.commissionPct}%`} · −${money(t.commissionEx)}</div>` : ''}</td>
...
window.openBoardEdit = async (id) => {
  ...
  const rows = BOARD_ROLES.map(role => {
    ...
    return { role, name: (cur && cur.name) || '', priceExVat: (cur && cur.priceExVat) || '',
      vatExempt: !!(cur && cur.vatExempt), priceIncVat: !!(cur && cur.priceIncVat), note: (cur && cur.note) || '',
      payableId: (cur && cur.paidPayableId) || null, payableNumber: (cur && cur.paidInvoice) || '', fixed: true };
  });
  const extras = ev ? (ev.rows || []).filter(r => !BOARD_ROLES.includes(r.role))
    .map(r => ({ role: r.role, name: r.name || '', priceExVat: r.priceExVat || '', vatExempt: !!r.vatExempt, priceIncVat: !!r.priceIncVat, note: r.note || '',
      payableId: r.paidPayableId || null, payableNumber: r.paidInvoice || '', fixed: false })) : [];
  _boardEdit = {
    id: ev ? ev.id : null,
...
function bdIncomeLine(e) {
  ...
  return `<div class="row-between"><span>מחיר ללקוח</span><b>${money(price)}</b></div>${rows}
    ...
    ${total > price ? '<div style="color:var(--danger);font-size:11.5px;margin-top:4px">העמלות גבוהות מהמחיר ללקוח — התשלום מוצג כאפס.</div>' : ''}`;
}

// בוחר הוצאה קיימת לשורת תפקיד. ממלא ספק, סכום ללא מע"מ וסימון עוסק פטור,
// ומקשר את ההוצאה לשורה — כך היא נכנסת גם למעקב התשלומים לספקים.
let _bdPick = null;
window.bdPickExpense = async (i) => {
  boardEditSync();
...
function renderBdPick() {
  const st = _bdPick; if (!st) return;
  let m = document.getElementById('bdPickModal');
  if (!m) { m = document.createElement('div'); m.id = 'bdPickModal'; m.className = 'modal'; document.body.appendChild(m); }
      ...
      : `<div style="max-height:52vh;overflow:auto;border:1px solid var(--line);border-radius:8px">
        ${list.map(x => `<label style="display:flex;gap:8px;align-items:center;font-size:12.5px;padding:7px 9px;border-top:1px solid var(--line);${x.linked ? 'opacity:.6' : ''}">
          ...
          <span style="flex:1;min-width:0"><b>${escapeHtml(x.supplierName || '—')}</b>
            ...
            ${x.description ? `<div class="muted" style="font-size:11px">${escapeHtml(x.description)}</div>` : ''}</span>
          ...
          <span style="white-space:nowrap;font-weight:600">${money(x.amountExcludeVat || x.amount)}</span></label>`).join('')}
      </div>`);
  m.innerHTML = `<div class="modal-card" style="width:min(680px,95vw)">
    ...
    <div class="modal-actions">
      <button class="btn ghost" onclick="document.getElementById('bdPickModal').classList.add('hidden')">ביטול</button>
...
window.bdPickConfirm = () => {
  const st = _bdPick; if (!st) return;
  const sel = document.querySelector('input[name="bdpick"]:checked');
  if (!sel) { alert('לא נבחרה הוצאה.'); return; }
  const x = (st.items || []).find(y => String(y.id) === sel.value); if (!x) return;
  const row = (_boardEdit.rows || [])[st.i]; if (!row) return;
  row.name = x.supplierName || row.name;
  // סכום ללא מע"מ מההוצאה. אם אין הפרדה — הסכום הוא הסופי, כלומר עוסק פטור.
  const ex = Number(x.amountExcludeVat) || 0, inc = Number(x.amount) || 0;
  if (ex > 0) { row.priceExVat = ex; row.vatExempt = false; row.priceIncVat = false; }
  else if (inc > 0) { row.priceExVat = inc; row.vatExempt = true; row.priceIncVat = false; }
  row.payableId = x.id;
  row.payableNumber = x.number || '';
  document.getElementById('bdPickModal').classList.add('hidden');
  renderBoardEdit();
};
...
function boardEditSync() {
  ...
  const g = (id) => document.getElementById(id);
  e.date = g('bdDate')?.value || e.date;
  e.artist = g('bdArtist')?.value ?? e.artist;
...
function renderBoardEdit() {
  const e = _boardEdit; if (!e) return;
  let m = document.getElementById('bdModal');
  ...
  const income = Math.max(0, (Number(e.price) || 0) - bdCommList(e).reduce((a, c) => a + c.amount, 0));
  const rowHtml = (r, i) => `<div class="bd-row" style="display:grid;grid-template-columns:96px 1fr 88px 52px 70px 1fr 82px 26px;gap:6px;align-items:center;padding:4px 0;border-top:1px solid var(--line)">
    ...
    <button class="btn ghost" style="padding:1px 7px;font-size:11px;white-space:nowrap" onclick="bdPickExpense(${i})" title="בחירה מהוצאות שכבר נקלטו במערכת">🔗 מהוצאות</button>
    ...
    ${r.payableId ? `<div style="grid-column:1/-1;font-size:11px;color:var(--accent2);margin-top:2px">🔗 משויך להוצאה ${escapeHtml(String(r.payableNumber || ''))} — הסכום והספק הושלמו ממנה</div>` : ''}
  </div>`;
  m.innerHTML = `<div class="modal-card tall-form" style="width:min(860px,96vw)">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:8px">
      <label style="font-size:12.5px">תאריך<input id="bdDate" type="date" value="${escAttr(e.date)}" style="width:100%;padding:6px 8px"/></label>
      <label style="font-size:12.5px">שם האירוע<input id="bdArtist" value="${escAttr(e.artist)}" placeholder="למשל: בת מצווה" style="width:100%;padding:6px 8px"/></label>
    ...
    <div class="bd-head" style="display:grid;grid-template-columns:96px 1fr 88px 52px 70px 1fr 82px 26px;gap:6px;font-size:11px;color:var(--muted);padding-bottom:2px">
      <span>תפקיד</span><span>ספק</span><span>מחיר</span><span>פטור</span><span>כולל מע״מ</span><span>הערה</span><span>הוצאה</span><span></span></div>
    ...
    <div class="modal-actions">
      ...
      <button class="btn primary" onclick="boardSave(this)">${e.id ? 'שמור' : 'צור אירוע'}</button>
    </div>
  </div>`;
}
window.boardRecalc = () => {
...
window.boardSave = async (btn) => {
  ...
  const r = await fetch('/api/event-board', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    .then(x => x.json()).catch(() => ({ error: 'שגיאת רשת' }));
  if (btn) btn.disabled = false;
  if (!r || r.error) { if (st) st.innerHTML = `<span style="color:var(--danger)">${escapeHtml(String((r && r.error) || 'שמירה נכשלה'))}</span>`; return; }
  document.getElementById('bdModal').classList.add('hidden');
  renderEventsBoard($('#content'));
};
window.boardDelete = async (id) => {
  if (!confirm('למחוק את האירוע וכל שורות ההוצאה שלו?')) return;
  const r = await fetch(`/api/event-board/${id}`, { method: 'DELETE' }).then(x => x.json()).catch(() => ({ error: 'שגיאת רשת' }));
  if (!r || r.error) { const st = document.getElementById('bdStatus'); if (st) st.innerHTML = `<span style="color:var(--danger)">${escapeHtml(String((r && r.error) || ''))}</span>`; return; }
  document.getElementById('bdModal').classList.add('hidden');
  renderEventsBoard($('#content'));
};
...
// עלות העובדים אינה שמורה על האירוע — התעריף היומי יושב בכרטיס העובד, והפירוק
// לתשלום/בונוס נעשה ב-payroll.js. לכן הדוח מושך את המשמרות מ-/api/payroll
// ומשתמש באותם מספרים שמופיעים במסך השכר, במקום לחשב אותם מחדש כאן.
const evShiftTotal = (s) => (Number(s.base) || 0) + (Number(s.bonus) || 0) + (Number(s.food) || 0) + (Number(s.travel) || 0);
const EV_FACTOR_LABEL = { 0.5: 'חצי יומית', 1: 'יומית', 1.5: 'יומית וחצי', 2: 'יומית כפולה' };
function evMonthReportHtml(monthKey, mode, events, shiftsByEvent) {
  ...
  const shiftsOf = (e) => (shiftsByEvent && shiftsByEvent.get(e.id)) || [];
  const havePay = Boolean(shiftsByEvent);
  const empOf = (e) => shiftsOf(e).reduce((t, s) => t + evShiftTotal(s), 0);
  const gross = r2(evs.reduce((s, e) => s + evGross(e), 0));
  const ctrCost = r2(evs.reduce((s, e) => s + ctrOf(e), 0));
  const empCost = r2(evs.reduce((s, e) => s + empOf(e), 0));
  ...
  const rows = evs.map(e => {
    const g = evGross(e), ctr = ctrOf(e), emp = empOf(e), st = evPayState(e);
    ...
    return `<tr>
      ...
      ${havePay ? `<td style="padding:5px 8px;border-bottom:1px solid #edeff7;text-align:left;white-space:nowrap;color:#b42318">${emp ? '−' + _repMoney(emp) : '—'}</td>` : ''}
      <td style="padding:5px 8px;border-bottom:1px solid #edeff7;text-align:left;white-space:nowrap;font-weight:700">${_repMoney(r2(g - ctr - emp))}</td>
  ...
  }).join('') || `<tr><td colspan="${havePay ? 10 : 9}" style="padding:10px;color:#6b7488">אין אירועים בחודש זה.</td></tr>`;

  // ---- פירוט מלא לכל אירוע ----
  // זה גוף הדוח: לכל אירוע בנפרד — מתוך מה מורכב המחיר, מי הקבלנים וכמה מהם
  // כבר שולמו, אילו עובדים עבדו ובכמה, אילו מסמכים הופקו, ומה נשאר בשורה התחתונה.
  const cell = (v, extra = '') => `<td style="padding:4px 8px;border-bottom:1px solid #edeff7;${extra}">${v}</td>`;
  const mini = (title, head, body) => `<div style="margin-top:8px">
    <div style="font-size:11.5px;font-weight:700;color:#4338ca;margin-bottom:3px">${title}</div>
    <table style="width:100%;border-collapse:collapse;font-size:11.5px">
      <thead><tr style="background:#f2f3fb">${head}</tr></thead><tbody>${body}</tbody></table></div>`;

  const details = evs.map(e => {
    const g = evGross(e), ctr = ctrOf(e), st = evPayState(e);
    const shifts = shiftsOf(e), emp = empOf(e);
    // מתוך מה מורכב המחיר — רק הרכיבים שמולאו
    const parts = [['הופעה', e.price], ['תאורה', e.priceLighting], ['סאונד', e.priceSound], ['בקליין', e.priceBackline],
      [`מסך לד${evLedQty(e) ? ` (${evLedQty(e)}מ׳)` : ''}`, (Number(e.ledPricePerMeter) || 0) * evLedQty(e)], ['תוספות', e.priceExtras]]
      .filter(([, v]) => Number(v) > 0)
      .map(([l, v]) => `<span style="display:inline-block;border:1px solid #d8dced;border-radius:6px;padding:1px 7px;margin:0 0 3px 4px;font-size:11px">${l} <b>${_repMoney(v)}</b></span>`).join('')
      || '<span style="color:#6b7488;font-size:11px">לא הוזן תמחור</span>';
    ...
    const docs = (e.linkedDocs || []).length
      ...
      : '<span style="color:#6b7488;font-size:11px">לא הופק מסמך</span>';

    return `<div style="border:1px solid #d8dced;border-radius:8px;padding:10px 12px;margin-bottom:10px;page-break-inside:avoid">
      ...
      <div style="margin-top:8px;padding-top:6px;border-top:1px solid #edeff7;font-size:12px">
        הכנסה <b>${_repMoney(g)}</b>
        ${ctr ? ` · קבלנים <b style="color:#b42318">−${_repMoney(ctr)}</b>` : ''}
        ${(havePay && emp) ? ` · עובדים <b style="color:#b42318">−${_repMoney(emp)}</b>` : ''}
        · נותר <b style="color:${(g - ctr - emp) >= 0 ? '#0a7d33' : '#b42318'}">${_repMoney(r2(g - ctr - emp))}</b>
        ${havePay ? '' : '<span style="color:#6b7488"> · ללא עלות עובדים</span>'}
      </div>
    </div>`;
  }).join('');
  ...
  return `${_repHead('דוח אירועים — ' + monthKeyLabel(monthKey), sub)}
    <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:16px">
      ...
      ${(havePay && empCost) ? _repRow('עלות עובדים', '−' + _repMoney(empCost), { color: '#b42318' }) : ''}
      ${(ctrCost || empCost) ? _repRow(havePay ? 'נותר אחרי קבלנים ועובדים' : 'סה״כ לאחר קבלנים',
        _repMoney(r2(gross - ctrCost - empCost)), { bold: true }) : ''}
    </table>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:18px">
      <thead><tr style="background:#eef0fb">
        ...
        ${havePay ? '<th style="padding:6px 8px;text-align:left">עובדים</th>' : ''}
        <th style="padding:6px 8px;text-align:left">נותר</th><th style="padding:6px 8px;text-align:right">מסמכים</th>
      ...
      <tfoot><tr style="font-weight:700;background:#f7f8fc">
        ...
        ${havePay ? `<td style="padding:6px 8px;text-align:left">${empCost ? '−' + _repMoney(empCost) : '—'}</td>` : ''}
        <td style="padding:6px 8px;text-align:left">${_repMoney(r2(gross - ctrCost - empCost))}</td>
        <td colspan="2"></td></tr></tfoot>
    </table>
    <div style="font-size:14px;font-weight:700;margin:0 0 8px">פירוט לכל אירוע</div>
    ${details || '<div style="color:#6b7488;font-size:12px">אין אירועים בחודש זה.</div>'}
    ${sup ? `<div style="font-size:14px;font-weight:700;margin:14px 0 6px">נספח — ריכוז קבלנים לחודש</div>
    <table style="width:100%;border-collapse:collapse;font-size:12.5px;margin-bottom:16px">
      <thead><tr style="background:#eef0fb">
...
window.evMonthReport = async (mode, monthKey, btn) => {
  const list = _evMonthGroups[_evmKey(mode, monthKey)];
  if (!list || !list.length) return;
  // עלות העובדים מגיעה ממסך השכר, כדי שהדוח והשכר יראו אותם מספרים. אם השליפה
  // נכשלת (למשל משתמש צפייה) — הדוח יוצא בלי הסכומים, ולא נכשל.
  let shiftsByEvent = null;
  if (/^\d{4}-\d{2}$/.test(String(monthKey))) {
    if (btn) { btn.disabled = true; btn.textContent = 'מכין דוח…'; }
    try {
      const pay = await api(`/api/payroll?companyId=${state.company}&month=${monthKey}`);
      if (Array.isArray(pay)) {
        shiftsByEvent = new Map();
        for (const emp of pay) for (const s of (emp.shifts || [])) {
          if (!s.eventId) continue;
          const arr = shiftsByEvent.get(s.eventId) || [];
          arr.push({ ...s, name: emp.name });
          shiftsByEvent.set(s.eventId, arr);
        }
      }
    } catch { shiftsByEvent = null; }
    if (btn) { btn.disabled = false; btn.textContent = '📄 דוח חודשי'; }
  }
  _boardPdf(evMonthReportHtml(monthKey, mode, list, shiftsByEvent), `דוח אירועים - ${monthKeyLabel(monthKey)}.pdf`, btn);
};
...
window.openBoardView = (id, keepOpen) => {
  ...
  const docs = (ev.linkedDocs || []).length ? (ev.linkedDocs || []).map(d => {
    ...
    const state = d.credit ? '<span class="tag" style="background:#fde8e8;color:#b42318">זיכוי</span>'
      : d.converted ? '<span class="tag" style="background:#eef0fb;color:#5b6180">הומר</span>'
      : '<span class="tag invoiced">פעיל</span>';
    const view = d.uploaded
      ? `<button class="btn ghost" style="padding:2px 9px;font-size:11.5px" onclick="previewDoc('/api/files/${encodeURIComponent(d.id)}')">👁 צפייה</button>
         <a class="btn ghost" style="padding:2px 9px;font-size:11.5px;text-decoration:none" href="/api/files/${encodeURIComponent(d.id)}" download target="_blank" rel="noopener">⬇ הורדה</a>`
      : `<button class="btn ghost" style="padding:2px 9px;font-size:11.5px" onclick="previewLinkedDoc('${d.id}',this,'${ev.id}')">👁 צפייה</button>
         <a class="btn ghost" style="padding:2px 9px;font-size:11.5px;text-decoration:none" href="/api/documents/${encodeURIComponent(d.id)}/download" download target="_blank" rel="noopener">⬇ הורדה</a>`;
  ...
  m.innerHTML = `<div class="modal-card bv-split" style="width:${shown ? 'min(1480px,98vw)' : 'min(1000px,97vw)'};max-height:92vh;max-height:92dvh;overflow:hidden;display:flex;gap:14px;align-items:stretch">
    ...
    <div style="margin-bottom:12px;padding:10px 12px;border:1px solid var(--line);border-radius:10px;font-size:13px;line-height:1.9">
      ...
      ${t.commissionOver ? '<div style="color:var(--danger);font-size:11.5px;margin-top:4px">העמלות גבוהות מהמחיר ללקוח — התשלום מוצג כאפס.</div>' : ''}
    </div>
    <div class="cards" style="margin-bottom:14px">
      ...
      <div class="card"><div class="label">רווח</div><div class="big" style="color:${(t.profitEx || 0) >= 0 ? 'var(--accent2)' : 'var(--danger)'}">${money(t.profitEx || 0)}</div><div class="muted" style="font-size:11px">${t.unpaidRows || 0} שורות טרם שולמו</div></div>
    </div>
    ...
    <div class="row-between" style="margin:16px 0 4px">
      <div style="font-size:13px;font-weight:700">מסמכים ללקוח</div>
      <button class="btn primary" style="padding:3px 11px;font-size:12.5px" onclick="boardIssueDoc('${ev.id}')">🧾 הפקת מסמך לאירוע</button>
    </div>
    <table class="tbl no-cardify" style="width:100%;font-size:12.5px">
      <thead><tr><th>מסמך</th><th>מצב</th><th style="text-align:left"></th></tr></thead>
      <tbody>${docs}</tbody>
    </table>
...
window.boardIssueDoc = async (id) => {
  const ev = boardFind(id); if (!ev) return;
  const bm = document.getElementById('bvModal'); if (bm) bm.classList.add('hidden');
  const mm = document.getElementById('newQuoteModal') || (() => { const x = document.createElement('div'); x.id = 'newQuoteModal'; x.className = 'modal'; document.body.appendChild(x); return x; })();
  mm.classList.remove('hidden');
  mm.innerHTML = `<div class="modal-card" style="width:min(720px,96vw)"><div class="empty">טוען לקוחות…</div></div>`;
  try { const cl = await api('/api/clients'); if (Array.isArray(cl) && cl.length) _evClients = cl; } catch { if (!_evClients) _evClients = []; }
  ...
  _nq = {
    ...
    boardNote: (t.commissionEx > 0)
      ? `הסכום הוא לאחר ${(t.commissions || []).map(c => `${c.name}${c.pct != null ? ` ${c.pct}%` : ''} ${money(c.amount)}`).join(' + ')} — מתוך ${money(t.clientPriceEx)}.`
      : '',
...
window.loadMigrateReport = async (btn) => {
  ...
  box.innerHTML = `${blocked}
    <div style="border:1px solid var(--line);border-radius:12px;padding:12px">
      ...
      <div class="muted" style="font-size:12.5px">העברה לרו"ח: ${r.accountant && r.accountant.email
        ? `${escapeHtml(r.accountant.email)}${r.accountant.mailConfigured ? '' : ' — אך אין חשבון מייל לחברה'}`
        : 'לא הוגדרה כתובת'}</div>
